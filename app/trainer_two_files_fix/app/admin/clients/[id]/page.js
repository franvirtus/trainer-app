                                        ...
                                        {session.setRows?.some((row) => row.reps !== null || row.kg !== null) ? (
                                          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                                            ...
                                            <div className="space-y-2">
                                              {session.setRows.map((row) => (
                                                <div
                                                  key={`${session.base_log_id}_${row.set_index}`}
                                                  className="grid grid-cols-[52px_1fr_1fr] items-center text-sm font-mono bg-white border border-slate-200 rounded-xl px-3 py-2"
                                                >
                                                  <span className="font-black text-slate-400">#{row.set_index}</span>
                                                  <span className="font-black text-slate-900 text-center">
                                                    {row.reps !== null ? `${row.reps} reps` : "-"}
                                                  </span>
                                                  <span className="font-black text-emerald-700 text-center">
