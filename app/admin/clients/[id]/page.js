...
export default function ClientPage({ params }) {
  ...
  const [bodyForm, setBodyForm] = useState({
    neck: "", chest: "", waist: "", hips: "", thigh: "", arm: "", calf: ""
  });
  ...
  const addWeight = async () => {
    ...
    await supabase.from("clients").update({ current_weight_kg: w }).eq("id", id); // Aggiorna anche l'anagrafica
  ...
  const saveBodyMeasures = async () => {
    ...
    await supabase.from("client_measurements").insert([payload]);
  ...
  const createProgram = async () => {
    ...
    const { data, error } = await supabase.from("programs").insert([{ 
        client_id: id, title: cpTitle, coach_name: coachName, duration: Number(cpDuration), status: 'active'
    }]).select().single();
  ...
  const copyLink = (pid) => {
    ...
    else { navigator.clipboard.writeText(link); alert("Link copiato negli appunti!"); }
  };

  if (loading) return <div className="min-h-screen flex items-center justify-center text-slate-400 font-medium">Caricamento profilo...</div>;
          ...
          <div className="max-w-6xl mx-auto px-6 py-4">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <button onClick={() => router.push("/admin/dashboard")} className="p-2 hover:bg-slate-100 rounded-full transition text-slate-500">
                        <ArrowLeft size={22} />
                    </button>
                    <div>
                ...
                <div className="xl:col-span-1 space-y-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <select value={profile.gender} onChange={e => setProfile({...profile, gender: e.target.value})} className="w-full mt-1 bg-slate-50 border border-slate-200 p-3 rounded-xl font-medium text-slate-700 outline-none focus:border-blue-500 bg-white">
                                ...
                                <div>
                                    ...
                                    <input type="number" value={profile.height_cm} onChange={e => setProfile({...profile, height_cm: e.target.value})} className="w-full mt-1 bg-slate-50 border border-slate-200 p-3 rounded-xl font-medium text-slate-700 outline-none focus:border-blue-500"/>
                                </div>
                            </div>
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase">Email</label>
                                <input value={profile.email} onChange={e => setProfile({...profile, email: e.target.value})} className="w-full mt-1 bg-slate-50 border border-slate-200 p-3 rounded-xl font-medium text-slate-700 outline-none focus:border-blue-500"/>
                            </div>
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase">Obiettivo</label>
                                <textarea rows={3} value={profile.goal} onChange={e => setProfile({...profile, goal: e.target.value})} className="w-full mt-1 bg-slate-50 border border-slate-200 p-3 rounded-xl font-medium text-slate-700 outline-none focus:border-blue-500 resize-none"/>
                            </div>
                            <button onClick={saveProfile} disabled={savingProfile} className="w-full bg-slate-900 text-white py-3 rounded-xl font-bold hover:bg-black transition mt-2 disabled:opacity-50 flex justify-center items-center gap-2">
                    ...
                    <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
                        ...
                        <div className="flex gap-2 mb-4">
                            <input type="number" placeholder="kg" className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-bold text-lg outline-none focus:bg-white focus:border-blue-500 transition"
                                value={newWeight} onChange={(e) => setNewWeight(e.target.value)} />
                            <button onClick={addWeight} disabled={savingWeight} className="bg-blue-600 text-white px-5 rounded-xl font-bold hover:bg-blue-700 transition disabled:opacity-50"><Plus/></button>
                        </div>
                        <div className="space-y-2 max-h-[250px] overflow-y-auto pr-1 custom-scrollbar">
                            {metrics.map(m => (
                                <div key={m.id} className="flex justify-between items-center p-3 rounded-xl bg-slate-50 border border-slate-100">
                            ...
                            {metrics.length === 0 && <p className="text-center text-slate-400 text-sm py-2">Nessun peso registrato.</p>}
                        </div>
                    </div>
                </div>
                ...
                <div className="xl:col-span-2 space-y-6">
                    <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
                        <div className="flex justify-between items-center mb-4 border-b border-slate-100 pb-2">
                            ...
                            <button onClick={saveBodyMeasures} disabled={savingBody} className="text-xs font-bold bg-indigo-50 text-indigo-600 px-4 py-2 rounded-lg hover:bg-indigo-100 transition">
                            ...
                            <table className="w-full text-sm text-left">
                                <thead className="text-[10px] text-slate-400 uppercase font-bold border-b border-slate-100 bg-slate-50/50">
                                    <tr>
                                        ...
                                        <th className="py-3 rounded-tr-xl">Coscia</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-50">
                                    {bodyMeasuresHistory.length === 0 ? (
                                        <tr><td colSpan="6" className="py-6 text-center text-slate-400 italic">Nessuna misurazione salvata.</td></tr>
                                    ) : (
            ...
            <div className="space-y-4">
                {programs.length === 0 ? (
                    ...
                    <div className="grid gap-4">
                        {programs.map(p => (
                            <div key={p.id} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:border-blue-300 transition-all group flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                                <div>
                    ...
                    {recentLogs.length === 0 ? <p className="text-slate-400 text-sm italic text-center py-10">Nessun log registrato finora.</p> : 
                     recentLogs.map(log => (
                        <div key={log.id} className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-50 pb-4 last:border-0 last:pb-0 gap-2">
                            <div>
                                <div className="font-bold text-slate-800 text-base capitalize flex items-center gap-2">
                                    {log.exercise_name}
                                    <span className="text-[9px] uppercase font-bold bg-slate-100 text-slate-500 px-2 py-0.5 rounded border border-slate-200">{log.day_label}</span>
                                </div>
                                <div className="text-xs text-slate-400 mt-0.5 flex items-center gap-1"><Calendar size={10}/> {new Date(log.created_at).toLocaleDateString()} &bull; {new Date(log.created_at).toLocaleTimeString().slice(0,5)}</div>
                            </div>
                            <div className="flex items-center gap-4 bg-slate-50 px-4 py-2 rounded-xl border border-slate-100 self-start sm:self-auto w-full sm:w-auto justify-between sm:justify-start">
                                <div className="text-center">
                                    <span className="block text-[9px] font-bold text-slate-400 uppercase">Set</span>
                                    <span className="font-bold text-slate-700 text-sm">{log.actual_sets || "-"}</span>
                                </div>
                                <div className="text-center">
                                    <span className="block text-[9px] font-bold text-slate-400 uppercase">Reps</span>
                                    <span className="font-bold text-slate-900 text-sm font-mono">{log.actual_reps}</span>
                                </div>
                                <div className="text-center">
                                    <span className="block text-[9px] font-bold text-slate-400 uppercase">Carico</span>
                                    <span className="font-bold text-green-600 text-sm font-mono">{log.actual_weight} Kg</span>
                                </div>
                            </div>
                        </div>
                    ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
           <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl p-6 animate-in fade-in zoom-in-95 duration-200">
               <div className="flex justify-between items-center mb-6">
                   ...
                   <button onClick={() => setShowCreateProgram(false)} className="p-2 bg-slate-100 rounded-full hover:bg-slate-200 text-slate-500 transition"><X size={20}/></button>
               </div>
               <div className="space-y-5">
                   <div>
                       <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1 block">Nome Programma</label>
                       <input autoFocus value={cpTitle} onChange={e => setCpTitle(e.target.value)} placeholder="Es. Ipertrofia Base" className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl font-bold text-lg outline-none focus:border-blue-500 focus:bg-white transition placeholder:font-normal"/>
                   </div>
                   <div>
                       <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1 block">Durata (Settimane)</label>
                       <input type="number" value={cpDuration} onChange={e => setCpDuration(e.target.value)} className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl font-bold text-lg outline-none focus:border-blue-500 focus:bg-white transition"/>
                   </div>
                   <div className="pt-2">
                       <button onClick={createProgram} disabled={creatingProgram} className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold hover:bg-black transition shadow-lg shadow-slate-300 disabled:opacity-50 flex justify-center gap-2">
                           {creatingProgram ? "Creazione..." : <><Plus size={20}/> Crea Scheda</>}
                       </button>
                   </div>
               </div>
           </div>
