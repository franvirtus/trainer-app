...
import {
  ArrowLeft, Save, Plus, Trash2, Copy, GripVertical, MoreVertical, Settings, Calendar, Info, FileText
} from "lucide-react";
...
export default function EditorPage({ params }) {
  ...
  const [days, setDays] = useState([]); // Array di giorni
  ...
  const fetchProgram = async () => {
    ...
    // Se esiste già una struttura JSON salvata, usala. Altrimenti crea Giorno A vuoto.
    if (data.days_structure && Array.isArray(data.days_structure) && data.days_structure.length > 0) {
      setDays(data.days_structure);
    } else {
  ...
  // --- AZIONI SUI GIORNI ---
  const addDay = () => {
    const newDayLabel = String.fromCharCode(65 + days.length); // A, B, C...
    const newDay = { 
      id: `day-${Date.now()}`, 
      name: `Giorno ${newDayLabel}`, 
      notes: '', // Nota della giornata vuota
      exercises: [] 
    };
    setDays([...days, newDay]);
    setActiveDayIndex(days.length); // Sposta il focus sul nuovo giorno
  };
  ...
  // --- NUOVA FUNZIONE: AGGIORNA NOTE GIORNATA ---
            ...
            <div className="p-5 space-y-4">
                {activeDay.exercises.length === 0 ? (
                    ...
                    activeDay.exercises.map((ex, idx) => (
                        <div key={ex.id} className="group relative bg-white border border-slate-200 rounded-2xl p-4 shadow-sm hover:border-blue-400 hover:shadow-md transition-all">
                            ...
                            {/* NOTE TECNICHE VISIBILI (NUOVO STILE) */}
