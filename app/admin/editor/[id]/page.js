  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans pb-20">
      <div className="bg-white border-b border-slate-200 px-6 py-4 sticky top-0 z-30 shadow-sm flex items-center justify-between">
        ...
        <button onClick={saveProgram} disabled={saving} className="bg-slate-900 text-white px-6 py-2.5 rounded-xl font-bold flex items-center gap-2 hover:bg-black transition shadow-lg disabled:opacity-50"><Save size={18}/> {saving ? "Salvataggio..." : "SALVA"}</button>
      </div>
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="bg-slate-50 border-b border-slate-200 p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-3 flex-1">
                    ...
                    <input value={activeDay.name} onChange={(e) => updateDayName(e.target.value)} className="bg-transparent font-black text-xl text-slate-800 outline-none w-full placeholder:text-slate-300" placeholder="Nome Giorno (es. Gambe)"/>
                </div>
                <div className="flex items-center gap-2">
            ...
            <div className="px-5 pt-5">
                <div className="bg-amber-50 border border-amber-100 rounded-xl p-3 flex gap-3">
                    ...
                    <div className="flex-1">
                        ...
                        <textarea value={activeDay.notes || ''} onChange={(e) => updateDayNotes(e.target.value)} placeholder="Es. Focus tempo sotto tensione, recuperi rigidi..." className="w-full bg-transparent outline-none text-sm text-amber-900 placeholder:text-amber-400/70 font-medium resize-none h-auto min-h-[40px]" rows={2}/>
                    </div>
                </div>
            </div>
            ...
            <div className="p-5 space-y-4">
                {(!activeDay.exercises || activeDay.exercises.length === 0) ? (
                    <div className="text-center py-10 border-2 border-dashed border-slate-100 rounded-xl text-slate-400"><Dumbbell size={32} className="mx-auto mb-2 opacity-20"/><p className="text-sm font-medium">Nessun esercizio.</p><p className="text-xs">Clicca "Aggiungi esercizio" qui sotto.</p></div>
                ) : (
