...
export default function AthleteView() {
  ...
  const [dayNotes, setDayNotes] = useState({}); // Per le note specifiche del giorno
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (template) {
        ...
        if (Array.isArray(template.content)) {
          template.content.forEach((item, index) => {
            ...
            if (item.type === 'day_note') {
              ...
              return; // Non è un esercizio, quindi passiamo al prossimo
            }
            ...
            // Se l'esercizio ha il formato "GIORNO X - Nome Esercizio"
            if (item.Esercizio.includes(" - ")) {
              const parts = item.Esercizio.split(" - ");
              dayName = parts[0].trim();
              // Rendiamo il nome del giorno bello da vedere (es. Giorno A)
        ...
        // Carica feedback storici dell'atleta
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans pb-10">
      ...
      <div className="px-4">
        ...
        {/* BOX NOTE SPECIFICHE DEL GIORNO (Se presenti) */}
        {dayNotes[activeTab] && (
